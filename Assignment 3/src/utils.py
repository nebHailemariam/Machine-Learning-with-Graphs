...
DATA_PATH = r"C:\Users\Nebiyou Hailemariam\Desktop\development\Machine-Learning-with-Graphs\Assignment 3\src\\data\\"
...
def global_page_rank_prediction(file_path, rank_file_path, user_id, query_id, type):
    ...
    with open(rank_file_path + rf"\predictions\GPR-{type}", "a") as output_file:
        idx = 1
...
def query_based_topic_sensitive_page_rank(
    ...
    ranks.sort(key=lambda x: -x[0])
    with open(
        rank_file_path + rf"\predictions\{user_or_query_topic}-{type}", "a"
    ) as output_file:
        idx = 1
        for rank_info in ranks:
            rank, user_id, query_id, Q0, DocID, score, RunID = rank_info
