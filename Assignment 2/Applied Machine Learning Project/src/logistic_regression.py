...
# Process development data using the same vocabulary
...
label = np.eye(num_classes)[np.array(dev_targets) - 1]

# Predictions on development data
dev_pred = np.argmax(model.forward(dev_features.T), axis=0)
dev_accuracy = np.mean(dev_pred == np.argmax(label.T, axis=0)) * 100
print("Dev Set Accuracy: {:.2f}%".format(dev_accuracy))

y_soft_pred_dev = get_soft_prediction(model, dev_features)
dev_mse = mean_squared_error(dev_targets, y_soft_pred_dev)
dev_rmse = np.sqrt(dev_mse)
print(f"Dev Set Root Mean Squared Error: {dev_rmse}")
